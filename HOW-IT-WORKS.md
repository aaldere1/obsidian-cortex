...
`brain-startup` returns that `session_id`; `brain-closeout` targets it with `idle --session <id>`. Two sessions on the same machine never overwrite each other's in-flight state.

...
Wiki workflow summary: `Synthesis/` is for LLM-maintained cross-cutting pages. `Sources/` is read-only raw sources. `index.md` is the catalog. `log.md` is the chronological record. See `<VAULT>/AGENTS.md` for the full workflow (ingest/query/lint).

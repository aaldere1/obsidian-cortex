...
Memory & sessions:
  ...
  node "AI Brain/scripts/brain.mjs" closeout "Project Name" "Session title" "Laptop" \
...
Jev gates (optional TypeSafe System One; fail-open; never writes prose):
  ...
  startup / closeout accept --use-jev / --no-jev (see AI Brain/docs/typesafe-jev-gates.md)
...
// Best-effort identity for THIS machine, used only when no machine name was passed.
//
// os.hostname() is NOT a reliable machine identity. With macOS `HostName` unset it
// falls back to a DHCP/Bonjour-derived name, which can collide with ANOTHER
// machine's folder: on Old-Laptop it returned literally "Mac", the folder belonging to Laptop.
// Trusting it routed Old-Laptop's records at Laptop. LocalHostName is stable, so prefer an
// alias hit on it, and shout if the two disagree.
async function localMachineIdentity() {
  const host = os.hostname();
  let localHost = "";
  try {
    localHost = execFileSync("scutil", ["--get", "LocalHostName"], {
      encoding: "utf8",
      stdio: ["ignore", "pipe", "ignore"],
    }).trim();
  } catch {
    // not macOS, or scutil unavailable — hostname is all we have
  }

  const aliases = await readMachineAliases();
  const folders = await machineDirectoryNames();
  const aliasHit = (value) => {
    if (!value) return "";
    for (const candidate of [`${value}.local`, value]) {
      const key = Object.keys(aliases).find((a) => a.toLowerCase() === candidate.toLowerCase());
      if (key) return resolveAliasChain(candidate, aliases);
    }
    return "";
  };
  const folderHit = (value) =>
    value ? folders.find((f) => f.toLowerCase() === String(value).toLowerCase()) || "" : "";

  const byLocal = aliasHit(localHost);
  const byHost = aliasHit(host);

  const localFolder = folderHit(byLocal) || folderHit(localHost);
  const hostFolder = folderHit(byHost) || folderHit(host);
  if (localFolder && hostFolder && localFolder !== hostFolder) {
    console.error(
      `warning: ambiguous machine identity — hostname "${host}" resolves to "${hostFolder}" ` +
        `but LocalHostName "${localHost}" resolves to "${localFolder}". Using "${localFolder}"; ` +
        `LocalHostName is the stable one. Pass the machine name explicitly to override.`
    );
  }

  return byLocal || localFolder || byHost || hostFolder || host;
}
...
async function whoami(machineArg) {
  ...
  const requested = (machineArg && machineArg.trim()) || (await localMachineIdentity());
...
function inlineOption(value, fallback) {
...
function omittedCloseoutField(flag) {
  return `Unknown — caller omitted --${flag}; reconcile from the source activity.`;
}

function markdownSectionItems(content, heading) {
...
function isRecordedDailyItem(value) {
  return !/^(?:none(?: recorded)?|todo\b|tbd\b|\(.*\))$/i.test(value.trim());
}
...
async function loadVaultProjectCandidates() {
  const projectsDir = path.join(brainRoot, "Projects");
  const activeMarkdown = await readTextIfExists(path.join(brainRoot, "Shared", "Active Projects.md"));
  const fromActive = parseActiveProjectNames(activeMarkdown);
  let fromDirs = [];
  if (await exists(projectsDir)) {
    fromDirs = (await readdir(projectsDir, { withFileTypes: true }))
      .filter((entry) => entry.isDirectory() && !entry.name.startsWith("_"))
      .map((entry) => entry.name);
  }
  return mergeProjectNames(fromActive, fromDirs);
}

function jevFlagsFromOptions(options) {
  return parseJevFlags(options);
}

async function maybeRefineStartupProject(project, options) {
  const result = await judgeProject({
    guess: project,
    cwd: options.cwd || process.cwd(),
    focus: options.focus || "",
    loadCandidates: loadVaultProjectCandidates,
    flags: jevFlagsFromOptions(options),
  });
  if (result.used && result.project) {
    console.log(`jev-project: ${project} → ${result.project} (${result.reason})`);
    return result.project;
  }
  if (result.enabled) {
    console.log(`jev-project: kept ${project} (${result.reason})`);
  }
  return project;
}

async function maybeAdviseCloseout(options) {
  const result = await judgeCloseoutRichness({
    hadCommits: options["had-commits"] === "true",
    hadFileEdits: options["had-edits"] === "true",
    hadPrOrShip: options["had-pr"] === "true",
    durationBand: options["duration-band"] || "unknown",
    flags: jevFlagsFromOptions(options),
  });
  if (!result.enabled) return;
  if (result.rich == null) {
    console.log(`jev-closeout: skipped (${result.reason}); closeout written`);
    return;
  }
  console.log(`jev-closeout: rich=${result.rich} noul=${result.noul} (advisory; closeout written)`);
}
...
async function jevStatusCommand(optionArgs) {
  printJson(await jevStatus({ flags: jevFlagsFromOptions(parseOptions(optionArgs)) }));
}

async function judgeProjectCommand(optionArgs) {
  const options = parseOptions(optionArgs);
  const candidates = options.candidates
    ? String(options.candidates).split(",").map((name) => name.trim()).filter(Boolean)
    : undefined;
  printJson(await judgeProject({
    guess: options.guess || options.project || "",
    cwd: options.cwd || "",
    focus: options.focus || "",
    candidates,
    loadCandidates: candidates ? undefined : loadVaultProjectCandidates,
    flags: jevFlagsFromOptions(options),
  }));
}

async function judgeCloseoutCommand(optionArgs) {
  const options = parseOptions(optionArgs);
  printJson(await judgeCloseoutRichness({
    hadCommits: options["had-commits"] === "true",
    hadFileEdits: options["had-edits"] === "true",
    hadPrOrShip: options["had-pr"] === "true",
    durationBand: options["duration-band"] || "unknown",
...
async function judgeScrubCommand(optionArgs) {
  const options = parseOptions(optionArgs);
  printJson(await judgePersonalScrub({
    pathKind: options["path-kind"] || "unknown",
    contentClass: options["content-class"] || "unknown",
    filename: options.filename || "",
    destination: options.destination || "public-cortex",
    denyListed: options["deny-listed"] === "true",
    allowlistOnly: options["allowlist-only"] === "true",
    fileCount: options["file-count"],
    flags: jevFlagsFromOptions(options),
  }));
}
...
async function newSession(projectArg, titleArg, machineArg) {
  ...
  const machineName = await resolveMachineName(machineArg || (await localMachineIdentity()));
...
async function closeout(projectArg, titleArg, machineArg, optionArgs) {
  ...
  const fileName = `${date}-${time}-${slug(title) || "session"}.md`;
  const destinationPath = path.join(sessionsDir, fileName);
  const goal = multilineList(options.goal, omittedCloseoutField("goal"));
  const summary = multilineList(options.summary, omittedCloseoutField("summary"));
  const changes = multilineList(options.changes, omittedCloseoutField("changes"));
  const decisions = multilineList(options.decisions, omittedCloseoutField("decisions"));
  const questions = multilineList(options.questions, omittedCloseoutField("questions"));
  const nextSteps = multilineList(options.next, omittedCloseoutField("next"));
  const references = multilineList(options.refs, omittedCloseoutField("refs"));
...
- Project: ${projectName}
- Summary: ${inlineOption(options.summary, omittedCloseoutField("summary"))}
- Files or areas touched: ${inlineOption(options.changes, omittedCloseoutField("changes"))}
- Decisions: ${inlineOption(options.decisions, omittedCloseoutField("decisions"))}
- Next: ${inlineOption(options.next, omittedCloseoutField("next"))}
`;
  ...
  await maybeAdviseCloseout(options);
}
...
\`\`\`sh
node "${scriptPath}" closeout "${projectName}" "Short session title" "${machineName}" \
  --goal "What this session set out to complete" \
  --summary "What changed and what was verified" \
  --changes "Files, repos, or areas touched" \
  --decisions "Important decisions, or None" \
...
async function startup(machineArg, optionArgs) {
  ...
  const project = await maybeRefineStartupProject(options.project || "Unknown", options);
...
async function daily(machineArg) {
  ...
  for (const session of sessionsToday) {
    const sessionPath = path.join(projectsDir, session.projectName, "Sessions", session.file);
    const sessionContent = await readTextIfExists(sessionPath);
    for (const [section, heading] of [
      ["decisions", "Decisions"],
      ["questions", "Open Questions"],
      ["nextSteps", "Next Steps"],
    ]) {
      for (const item of markdownSectionItems(sessionContent, heading).filter(isRecordedDailyItem)) {
        dailyItems[section].push(`- [${session.projectName}] ${item}`);
      }
    }
  }
  const decisionsBlock = dailyItems.decisions.length
    ? dailyItems.decisions.join("\n")
    : "- None recorded in today's session closeouts.";
  const openLoopsBlock = dailyItems.questions.length
    ? dailyItems.questions.join("\n")
    : "- None recorded in today's session closeouts.";
  const tomorrowBlock = dailyItems.nextSteps.length
    ? dailyItems.nextSteps.join("\n")
    : "- None recorded in today's session closeouts.";
...
async function installClaudeHook(optionArgs) {
  ...
  // A fresh Claude Code install has ~/.claude/ but often no settings.json until a
  // setting is changed — start from an empty object rather than refusing.
  if (!(await exists(path.dirname(settingsPath)))) {
    console.error(`Claude Code directory not found at: ${path.dirname(settingsPath)}`);
    console.error("Is Claude Code installed and has it been run at least once?");
    process.exit(1);
  }

  const settingsRaw = (await exists(settingsPath)) ? await readFile(settingsPath, "utf8") : "{}";
  let settings;
  try {
...
switch (command) {
  ...
  case "status":
    ...
    break;
  case "jev-status":
    await jevStatusCommand(args.slice(1));
    break;
  case "judge-project":
    await judgeProjectCommand(args.slice(1));
    break;
  case "judge-closeout":
    await judgeCloseoutCommand(args.slice(1));
    break;
  case "judge-scrub":
    await judgeScrubCommand(args.slice(1));
    break;
  case "startup":
